namespace MidSpace.MySampleMod.Commands
{
    ...
    public class CommandTest10 : ChatCommand
    {
        public CommandTest10()
            : base(ChatCommandSecurity.User, ChatCommandAccessibility.Server, "test10", new[] { "/test10" })
        {
        }
        ...
        public override void Help(ulong steamId, bool brief)
        {
            if (brief)
            {
                MyAPIGateway.Utilities.SendMessage(steamId, "/test10", "Test 10 short description. Server Command.");
            }
            else
            {
                var description = $@"Test 10 long description. Server Command.
Date: {DateTime.Now}";

                MyAPIGateway.Utilities.SendMissionScreen(steamId, "/Test10", null, " ", description, null, "OK");
            }
        }
        ...
        public override bool Invoke(ChatData chatData)
        {
            ...
            MyAPIGateway.Utilities.SendMissionScreen(chatData.SenderSteamId, "/Test3", null, " ", info.ToString(), null, "OK");
